...
public sealed class InviteRenderService(RuleEngine ruleEngine, IConfiguration config) : IInviteRenderer
{
    ...
    /// <summary>
    /// Whether this guest is offered the camera: they said they were coming, and it is the night.
    ///
    /// <para>The window itself is <see cref="EventDayWindow"/> — shared with the media bucket the
    /// camera shoots into, deliberately. Answered separately they drift, and the way that shows up is
    /// a camera offered on an invitation that leads to a bucket refusing every photo taken with
    /// it.</para>
    ///
